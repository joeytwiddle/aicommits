...
const getPrompt = (locale: string, diff: string) => `Write an insightful but concise (fewer than 12 words) Git commit message in a complete sentence in present tense for the following diff without prefacing it with anything, the response must be in the language ${locale}. Here is the diff:\n\n${diff}`;
